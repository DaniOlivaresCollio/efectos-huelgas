...
Creamos la variable *tipo factor* `actividad`. 
...
data <- filter(data,actividad %in% c("Trabaja de manera remunerada con jornada completa",
                                     "Trabaja de manera remunerada a tiempo parcial",
                                     "Estudia y trabaja"))
# Al hacer esto tbn se quitan los NA (ex -999,-888) 24 y 20 respectivamente.
```
...
Creamos la variable *tipo factor* `rel_empleo`.
...
Se crean dos nuevas variables tipo numéricas `isco08_1d` y `isco08_2d`, de la ISCO08 con 1 y 2 dígitos respectivamente. 
...
Se crea una nueva variable *tipo factor* con el Nivel de cualificación, llamada `cualificacion`, de acuerdo con la variable CIUO08 de 2 dígitos, `isco08_2d`, siguiendo el criterio:
...
Se crea la variable tipo factor `part_huelga`.
...
Se puede seguir usando la variable numérica `c08_03` para conseguir la misma tabla.
...
### Dummy participación en huelgas

Se crea una nueva variable dicotómica, llamada `participa_h`, para identificar a aquellas personas que:

-   `participa_h==1`: Participan en huelgas (`c08_03==2`, `c08_03==3`, `c08_03==4`, `c08_03==5`)
-   `participa_h==0`: No participan en huelgas (`c08_03==1`)
